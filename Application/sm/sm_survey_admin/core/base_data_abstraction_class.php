...
class base_data_abstraction
{
    ...
    function connect_db()
    {
        ...
        $this->mysqli = new mysqli($this->db_host, $this->db_user, $this->db_pass, $this->db_use, $this->db_port);
    ...
    function exec_fetch($result_type='array', $log=FALSE)
    {
        ...
        //Valid types are 'single', 'array', and 'rowdump'.
        //Default is 'array', and for robustness any other value simply gets treated as 'array'; this is a column-based result set.
        //'single' means only a single value will be returned, no need to make the results an array
        //'rowdump' means the dump array will contain data per row (row-based), instead of per column

        //Result = single record, no need for arrays to store the result set
        $result_type = strtolower($result_type);
    ...
    function stmt_prepare($bind_params=array())
    {
        ...
        else //Result = multiple records, store in arrays
        {
            ...
            while($result->fetch())
            {
                for($b=0; $b<$num_fields; ++$b)
                {
                    ...
                    if($result_type == 'rowdump')
                    {
                        init_var($this->dump[$a]);
                        if(is_array($this->dump[$a])) ;
                        else $this->dump[$a] = array();
                        $this->dump[$a][$field_name] = $value;
                    }
                    else
                    {
